...
with open('results/retention_output_jsd.txt', 'w', encoding='utf-8') as f:
    print('dataset', 'architecture', 'distractor_name', 'cap_len', 'token_pos', 'jsd', sep='\t', file=f)
            ...
            for (cap, img, distractors) in zip(captions, images, img_dists[dataset_name]):
                ...
                for (dist_name, dist_index) in distractors.items():
                    ...
                    for i in range(cap_len):
                        output_entries[(dist_name, cap_len-1, i)]['jsd'].append(jsd(outputs[0,i], outputs[1,i]))
                        
        with open('results/retention_output_jsd.txt', 'a', encoding='utf-8') as f:
            for (distractor_name, cap_len, token_pos) in sorted(output_entries.keys()):
                ...
                print(dataset_name, architecture, distractor_name, cap_len, token_pos, np.mean(dists['jsd']), sep='\t', file=f)