...
with open('results/retention_logits.txt', 'w', encoding='utf-8') as f:
...
for dataset_name in [ 'mscoco' ]:# 'flickr8k', 'flickr30k',
    ...
    for architecture in [ 'init', 'pre', 'par', 'merge' ]:
        logits_entries = collections.defaultdict(lambda:{ 'chebyshev': list(), 'cosine': list() })
            ...
            for (cap, img, distractors) in zip(captions, images, img_dists[dataset_name]):
                ...
                for (dist_name, dist_index) in distractors.items():
                    ...
                    for i in range(cap_len):
                        logits_entries[(dist_name, cap_len-1, i)]['chebyshev'].append(distance.chebyshev(logits[0,i], logits[1,i]))
                        logits_entries[(dist_name, cap_len-1, i)]['cosine'].append(distance.cosine(logits[0,i], logits[1,i]))
                        
        with open('results/retention_logits.txt', 'a', encoding='utf-8') as f:
            for (distractor_name, cap_len, token_pos) in sorted(logits_entries.keys()):
                dists = logits_entries[(distractor_name, cap_len, token_pos)]
